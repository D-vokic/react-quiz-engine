...
## 10. VERIFIED INCONSISTENCY (IMPORTANT)

### UI–DATA SEMANTIC MISMATCH

- `LastResultModal` expects:
  - `result.weakCount`

- `lastResult` generated by quiz logic contains only:
  - `score`
  - `accuracy`

This does **not** cause a runtime error, but **is a semantic inconsistency** between UI expectations and data model.

This is the **only known inconsistency** in the system.
